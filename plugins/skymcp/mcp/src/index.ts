...
server.tool(
  "sky_status",
  "List all SkyPilot clusters, managed jobs, and services with their current status",
  async () => {
...
server.tool(
  "sky_launch",
  "Launch a SkyPilot cluster or managed job from a YAML spec",
  {
    yaml_path: z
      .string()
      .optional()
    ...
    yaml_content: z
      ...
      .optional()
      .describe(
        "Inline YAML content (will be written to a temp file). Use this OR yaml_path, not both."
      ),
    cluster_name: z
      .string()
      .optional()
      .describe(
        "Cluster name for sky launch -c. If omitted, runs as managed job via sky jobs launch."
      ),
    extra_flags: z
      .string()
      .optional()
      .describe(
        "Additional CLI flags (e.g. '--use-spot --idle-minutes-to-autostop 30')"
      ),
    env_vars: z
      .string()
      .optional()
      .describe(
        'JSON-encoded key-value pairs for --env flags, e.g. \'{"WANDB_API_KEY":"abc","HF_TOKEN":"xyz"}\''
      ),
    dryrun: z
      .boolean()
      .optional()
  ...
  async ({ yaml_path, yaml_content, cluster_name, extra_flags, env_vars: env_vars_json, dryrun }) => {
    const env_vars: Record<string, string> | undefined = env_vars_json
...
server.tool(
  "sky_logs",
  "Retrieve recent logs from a SkyPilot cluster or managed job",
  {
    job_id: z
      .string()
      .optional()
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              error:
                "Either job_id (for managed jobs) or cluster_name (for clusters) is required",
            }),
          },
        ],
        isError: true,
...
server.tool(
  "sky_down",
  "Tear down a SkyPilot cluster, cancel a managed job, or take down a service",
  {
    cluster_name: z
      .string()
      .optional()
  ...
  async ({ cluster_name, job_id, service_name, purge }) => {
        ...
        content: [
          {
            ...
            text: JSON.stringify({
              error:
                "One of cluster_name, job_id, or service_name is required",
            }),
          },
        ],
        isError: true,
    ...
    const results: Array<{ command: string; stdout: string; exitCode: number }> =
      [];
...
server.tool(
  "sky_cost",
  "Show SkyPilot cost report for all managed resources",
  async () => {
...
server.tool(
  "sky_gpus",
  "List GPU availability and pricing across clouds",
  {
    gpu_type: z
      .string()
      .optional()
      .describe(
        "GPU type with optional count, e.g. 'H100:8', 'A100', 'L4:4'. Omit for all GPUs."
      ),
    cloud: z
      .string()
      .optional()
...
server.tool(
  "sky_check",
  "Check which cloud credentials are configured and valid for SkyPilot",
  async () => {
